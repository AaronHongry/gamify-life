    ...
    return (
        <div className="flex flex-row gap-3 w-full h-full overflow-hidden">
            <AnimatePresence>
                ...
                <motion.button id="addTask" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="card-background h-10 w-10 self-center rounded-lg" onClick={() => handleAddTask(true)}>+</motion.button>
            </AnimatePresence>   
        </div>
    );
